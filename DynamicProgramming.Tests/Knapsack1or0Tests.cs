...
namespace DynamicProgramming.Tests
{
    ...
    public class Knapsack1or0Tests
    {
        ...
        public void GetMaxValue_OnValidParams_ReturnsExpectedResult(decimal maxWeight, decimal expectedValue, IEnumerable<KnapsackItem> knapsackItems, IEnumerable<KnapsackItem> expectedKnapsackItems)
        {
            // Act
            var actualResult = Knapsack1or0.GetMaxValue(maxWeight, knapsackItems.ToList());
            // Assert
            Assert.AreEqual(expectedValue, actualResult);
        }

        [TestCaseSource("KnapsackTestSource")]
        public void GetKnapsackItems_OnValidParams_ReturnsExpectedResult(decimal maxWeight, decimal expectedValue, IEnumerable<KnapsackItem> knapsackItems, IEnumerable<KnapsackItem> expectedKnapsackItems)
        {
            // Act
            var actualResult = Knapsack1or0.GetKnapsackItems(maxWeight, knapsackItems.ToList());
            // Assert
            CollectionAssert.AreEquivalent(expectedKnapsackItems, actualResult);
            Assert.AreEqual(expectedValue, actualResult.Sum(i => i.Value));
        }
    }
}
